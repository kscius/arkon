# CONAGUA smoke test (PowerShell) — estatal, municipal y contratista
param(
...
Write-Host "CONAGUA smoke test"
Write-Host "  WebBase=$WebBase"
Write-Host "  ApiBase=$ApiBase"

try {
    $spaResponse = Invoke-WebRequest -Uri "$WebBase/" -UseBasicParsing
    if ($spaResponse.StatusCode -ge 400) { throw "SPA not reachable" }
...
Test-JsonOk "$WebBase/api/health" "GET $WebBase/api/health (proxy)"
Test-JsonOk "$ApiBase/health" "GET $ApiBase/health (direct API)"

# --- Estatal ---
try {
    $estatalToken = Get-AuthToken "estatal@conagua.gob.mx"
...
# --- Municipal ---
try {
    $municipalToken = Get-AuthToken "municipal.centro@conagua.gob.mx"
...
# --- Contratista ---
try {
    $contratistaToken = Get-AuthToken "cce@conagua.gob.mx"
    ...
    $created = Invoke-Api -Method Post -Path "/avances/obra/$cObraId" -Headers $cHeaders -Body @{
        ...
        programado  = 50
        reportado   = 49
        actividades = "Avance creado por smoke-conagua.ps1"
